...
## Current phase
**F1 / M1.7-gitignore mergeado en `developer`** (430b63e).

## Next actions (roadmap)
1. **M1.8-font-montserrat** — issue #56: bundlear Montserrat-Regular+Bold o cambiar brand a system-ui (decisión de producto).
2. **M1.9-forgot-reset** — backlog original Forgot/reset password + deep links.
3. **M1.10-splash-tabs** — Splash animado + tabs base.
4. **F2.x** — Home + catálogo + producto.
5. **F3.x** — Cart + checkout + pagos.
6. **F4.x** — Pedidos + cancelaciones + rating.
7. **F5.x** — Chat + notificaciones in-app.
8. **F6.x** — Perfil + cupones + contenido estático.
9. **F7.x** — Plugins nativos + Android/iOS signing + QA + release.

## Handover

[RELEVO DE AGENTE]
- Fase actual: F1 / M1.7-gitignore mergeado en `developer` (430b63e). Issues #51, #52, #53, #54, #55, #57 cerradas. Tests: 232 verdes (+7 quality). Siguiente: M1.8-font-montserrat (#56).
